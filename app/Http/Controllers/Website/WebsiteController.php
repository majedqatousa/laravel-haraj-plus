...
class WebsiteController extends Controller
{
    ...
    public function verify(Request $request)
    {
        ...
        if (isset($user)) {
            $new = User::where('phone', $phone)->first();
            $new->update(['is_active' => 1 , 'connected'=>1]);
    ...
    public function verify2(Request $request)
    {
        ...
        if (isset($user)) {
            ...
            session()->flash('success', 'رقم الهاتف لا ينتمي لأي حساب ');
            return response()->json(['success' => 0, 'phone' => $phone]);
        }
    }
