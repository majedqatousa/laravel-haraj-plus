...
class HomeController extends Controller
{
    ...
    public function fillterData(Request $request)
    {
        ...
        $extra = "else statment ";
        if(is_null($fromPrice) && is_null($toPrice) && is_null($category_id) && is_null($city_id) && !is_null($fromDate) && !is_null($toDate)){
            $productCreated = Product::where("is_valid" , 1)
                    ...
                    $extra = "1 is statment "; 
        } else if(!empty($fromPrice) && !empty($toPrice) && empty($category_id) && empty($city_id) && empty($fromDate) && empty($toDate)){
            $productCreated = Product::where("is_valid" , 1)
                    // ->where('category_id', $category_id)
                    // ->where('city_id', $city_id)
                    ->whereBetween('price', [$fromPrice, $toPrice])
        ...
        //     ->get();
        // }else if (!is_null($fromDate)&& !is_null($toDate) && is_null($category_id)&&is_null( $city_id)&& is_null($fromPrice)&&is_null($toPrice)){
        //     $productCreated = Product::whereBetween('created_at', [$fromDate, $toDate])->where("is_valid" , 1)
